        ...
        <form action="<?php echo e(isset($invoice) ? route('admin.invoices.update', $invoice->id) : route('admin.payments.customer.invoice.save', $customer->id)); ?>" method="POST" id="invoiceForm" onsubmit="return confirmInvoiceSubmit()">
            ...
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-4">
                    <a href="<?php echo e(route('admin.payments.customer', ['user' => $customer->id, 'transaction_id' => request('transaction_id') ?? ($transactionId ?? null)])); ?>" 
                ...
                <div class="overflow-x-auto" id="invoiceTableContainer" style="<?php echo e(empty($items) ? 'display: none;' : ''); ?>">
                    <table class="w-full" id="invoiceItemsTable">
                        ...
                        <tbody id="invoiceItemsBody" class="divide-y divide-gray-700">
                            ...
                            <tr class="invoice-item-row" data-index="<?php echo e($index); ?>" data-item-type="<?php echo e($item['type']); ?>" data-has-payment-id="<?php echo e(isset($item['payment_id']) ? 'true' : 'false'); ?>">
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <?php if($item['type'] == 'extra' && !isset($item['payment_id'])): ?>
                                        <button type="button" 
                                        ...
                                        <?php elseif($item['type'] == 'extra' && isset($item['payment_id'])): ?>
                                        <button type="button" 
                                                onclick="saveExtraCharge(this)"
...
function saveExtraCharge(button) {
    ...
    // Get transaction ID
    const transactionId = '<?php echo e(request("transaction_id") ?? ($transactionId ?? "")); ?>';
    ...
    // Send AJAX request
    fetch('<?php echo e(route("admin.payments.extraCharge.save", $customer->id)); ?>', {
        method: 'POST',
        headers: {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\VALESBEACH_LATEST\ValesBeach\resources\views/invoices/customer-invoice-edit.blade.php ENDPATH**/ ?>