...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex space-x-2 bg-gray-800 p-2 rounded-lg shadow-xl">
            <a href="<?php echo e(route('staff.orders.index', array_merge(request()->except('tab'), ['tab' => 'active']))); ?>"
               class="flex-1 px-6 py-3 text-center font-semibold rounded-lg transition-all duration-200 <?php echo e(($tab ?? 'active') === 'active' ? 'bg-gradient-to-r from-yellow-600 to-yellow-700 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-gray-700'); ?>">
                <div class="flex items-center justify-center space-x-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                    <span>Active Orders</span>
                    <?php if(isset($activeCount) && $activeCount > 0): ?>
                    <span class="px-2 py-0.5 text-xs font-bold rounded-full <?php echo e(($tab ?? 'active') === 'active' ? 'bg-white text-yellow-600' : 'bg-yellow-600 text-white'); ?>"><?php echo e($activeCount); ?></span>
                    <?php endif; ?>
                </div>
            </a>
            <a href="<?php echo e(route('staff.orders.index', array_merge(request()->except('tab'), ['tab' => 'completed']))); ?>"
               class="flex-1 px-6 py-3 text-center font-semibold rounded-lg transition-all duration-200 <?php echo e(($tab ?? 'active') === 'completed' ? 'bg-gradient-to-r from-green-600 to-green-700 text-white shadow-lg' : 'text-gray-400 hover:text-white hover:bg-gray-700'); ?>">
                <div class="flex items-center justify-center space-x-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="px-2 py-0.5 text-xs font-bold rounded-full <?php echo e(($tab ?? 'active') === 'completed' ? 'bg-white text-green-600' : 'bg-green-600 text-white'); ?>"><?php echo e($completedCount); ?></span>
                    <?php endif; ?>
                </div>
            </a>
        </div>
    ...
    <div class="bg-gray-800 rounded-lg shadow-xl p-6 mb-6">
        <form method="GET" action="<?php echo e(route('staff.orders.index')); ?>" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="<?php echo e(route('staff.orders.index', ['tab' => $tab ?? 'active'])); ?>" 
...
function updateOrderStatus(orderId, newStatus, orderNumber) {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (newStatus === 'completed' && '<?php echo e($tab ?? "active"); ?>' === 'active') {
