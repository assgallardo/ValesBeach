    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between mb-2">
                ...
                <div class="flex items-center gap-3">
                    <form method="POST" action="<?php echo e(route('admin.payments.customer.complete', $customer->id)); ?>" 
                          onsubmit="return confirm('Are you sure you want to mark all payments in this transaction for <?php echo e($customer->name); ?> as completed? This will move them to the Completed Transactions view.')">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="transaction_id" value="<?php echo e(request('transaction_id')); ?>">
                        <button type="submit"
                    ...
                    <a href="<?php echo e(route('admin.payments.customer.invoice', ['user' => $customer->id, 'transaction_id' => request('transaction_id')])); ?>" 
        ...
        <div class="bg-gray-800 rounded-lg border border-gray-700 p-6 mb-6">
            <div class="flex items-start justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-green-50 mb-2"><?php echo e($customer->name); ?></h2>
                    <p class="text-gray-400 mb-4"><?php echo e($customer->email); ?></p>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm text-gray-400">
                            Member since <?php echo e($customer->created_at->format('M d, Y')); ?>
                ...
                <div class="text-right">
                    ...
                    <div class="text-3xl font-bold text-green-400" id="total-amount-display">
                        ₱<?php echo e(number_format($customer->payments->whereNotIn('status', ['cancelled'])->sum('amount'), 2)); ?>

                    </div>
                    <div class="text-sm text-gray-400 mt-1" id="transaction-count-display">
                        <?php echo e($customer->payments->whereNotIn('status', ['cancelled'])->count()); ?> transaction<?php echo e($customer->payments->whereNotIn('status', ['cancelled'])->count() !== 1 ? 's' : ''); ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
                ...
                <div class="text-2xl font-bold text-green-400" id="booking-amount-display">
                    ₱<?php echo e(number_format($customer->payments->where('booking_id', '!=', null)->whereNotIn('status', ['cancelled'])->sum('amount'), 2)); ?>

                </div>
                <div class="text-sm text-gray-400 mt-1">
                    <?php echo e($customer->payments->where('booking_id', '!=', null)->whereNotIn('status', ['cancelled'])->count()); ?> payment<?php echo e($customer->payments->where('booking_id', '!=', null)->whereNotIn('status', ['cancelled'])->count() !== 1 ? 's' : ''); ?>
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
                ...
                <div class="text-2xl font-bold text-green-400" id="service-amount-display">
                    ₱<?php echo e(number_format($customer->payments->where('service_request_id', '!=', null)->whereNotIn('status', ['cancelled'])->sum('amount'), 2)); ?>

                </div>
                <div class="text-sm text-gray-400 mt-1">
                    <?php echo e($customer->payments->where('service_request_id', '!=', null)->whereNotIn('status', ['cancelled'])->count()); ?> payment<?php echo e($customer->payments->where('service_request_id', '!=', null)->whereNotIn('status', ['cancelled'])->count() !== 1 ? 's' : ''); ?>
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
                ...
                <div class="text-2xl font-bold text-green-400" id="food-amount-display">
                    ₱<?php echo e(number_format($customer->payments->where('food_order_id', '!=', null)->whereNotIn('status', ['cancelled'])->sum('amount'), 2)); ?>

                </div>
                <div class="text-sm text-gray-400 mt-1">
                    <?php echo e($customer->payments->where('food_order_id', '!=', null)->whereNotIn('status', ['cancelled'])->count()); ?> order<?php echo e($customer->payments->where('food_order_id', '!=', null)->whereNotIn('status', ['cancelled'])->count() !== 1 ? 's' : ''); ?>
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-6">
                ...
                <div class="text-2xl font-bold text-green-400" id="extra-amount-display">
                    ₱<?php echo e(number_format($customer->payments->whereNull('booking_id')->whereNull('service_request_id')->whereNull('food_order_id')->whereNotIn('status', ['cancelled'])->sum('amount'), 2)); ?>

                </div>
                <div class="text-sm text-gray-400 mt-1">
                    <?php echo e($customer->payments->whereNull('booking_id')->whereNull('service_request_id')->whereNull('food_order_id')->whereNotIn('status', ['cancelled'])->count()); ?> charge<?php echo e($customer->payments->whereNull('booking_id')->whereNull('service_request_id')->whereNull('food_order_id')->whereNotIn('status', ['cancelled'])->count() !== 1 ? 's' : ''); ?>
                        ...
                        <tbody class="divide-y divide-gray-700">
                            ...
                            <tr class="hover:bg-gray-750 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <?php elseif(!$payment->booking && !$payment->serviceRequest && !$payment->foodOrder): ?>
                                        <?php
                                        ...
                                        <div class="flex items-start gap-2">
                                            ...
                                            <div>
                                                ...
                                                <?php endif; ?>
                                                <?php if(isset($paymentDetails['invoice_number'])): ?>
                                                    <div class="text-xs text-blue-400">Invoice: <?php echo e($paymentDetails['invoice_number']); ?></div>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    <?php else: ?>
                                ...
                                <!-- Status -->
                                <td class="px-4 py-3">
                                    <select onchange="updatePaymentStatus(<?php echo e($payment->id); ?>, this.value)" 
                                        ...
                                        <?php elseif($payment->status === 'pending'): ?> bg-yellow-600 text-white
                                        <?php elseif($payment->status === 'confirmed'): ?> bg-blue-600 text-white
                                        <?php elseif($payment->status === 'overdue'): ?> bg-orange-600 text-white
                                        <?php elseif($payment->status === 'cancelled'): ?> bg-red-600 text-white
                                        <?php else: ?> bg-gray-700 text-gray-300
                                        <?php endif; ?>">
                                        ...
                                        <option value="overdue" <?php echo e($payment->status === 'overdue' ? 'selected' : ''); ?>>Overdue</option>
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex gap-1">
                                        ...
                                        <?php if($canRefund): ?>
                                            <button onclick="showRefundModal(<?php echo e($payment->id); ?>, <?php echo e($remainingAmount); ?>)"
                                        ...
                                        <?php endif; ?>
                                        <?php if($payment->refund_amount > 0): ?>
                                            <form method="POST" action="<?php echo e(route('admin.payments.cancelRefund', $payment)); ?>" style="display:inline;">
                                                <?php echo csrf_field(); ?>
                                        ...
                                        <?php if(!$payment->booking && !$payment->serviceRequest && !$payment->foodOrder && strpos($payment->payment_reference, 'EXT-') === 0): ?>
                                            <button onclick="deleteExtraCharge(<?php echo e($payment->id); ?>, this)" 
                                        ...
                                        <?php if($payment->status === 'cancelled'): ?>
                                            <button onclick="deleteCancelledPayment(<?php echo e($payment->id); ?>, '<?php echo e($payment->payment_reference); ?>', this)" 
                                                    class="inline-flex items-center px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700 transition-colors" 
...
function deleteExtraCharge(paymentId, button) {
    ...
    const url = `<?php echo e(route('admin.payments.extraCharge.delete', ':id')); ?>`.replace(':id', paymentId);
...
function deleteCancelledPayment(paymentId, paymentRef, button) {
    ...
    const url = `<?php echo e(route('admin.payments.destroy', ':id')); ?>`.replace(':id', paymentId);
    const csrfToken = document.querySelector('meta[name="csrf-token"]');
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\VALESBEACH_LATEST\ValesBeach\resources\views/admin/payments/customer.blade.php ENDPATH**/ ?>