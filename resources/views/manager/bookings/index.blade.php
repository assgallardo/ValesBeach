    ...
    <div x-data="{ activeTab: '{{ request('type', 'all') }}' }" class="mb-8">
        ...
        <div class="bg-gray-800 rounded-t-lg">
            <div class="flex border-b border-gray-700 overflow-x-auto">
                <button @click="activeTab = 'all'; window.location.href = '{{ route('manager.bookings.index', ['type' => 'all'] + request()->except('type')) }}'" 
                ...
                <button @click="activeTab = 'event'; window.location.href = '{{ route('manager.bookings.index', ['type' => 'event'] + request()->except('type')) }}'" 
                    ...
                    <tbody class="divide-y divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-700 transition-colors" id="cottage-booking-row-{{ $cottageBooking->id }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3">
                                    <!-- View Details -->
                                    <a href="{{ route('manager.bookings.show', $cottageBooking) }}" 
                                       class="text-blue-400 hover:text-blue-300 transition-colors" title="View Details">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-700 transition-colors" id="event-booking-row-{{ $eventBooking->id }}">
                            <td class="px-6 py-4">
                                <div class="text-white font-medium">#{{ $eventBooking->id }}</div>
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs font-medium rounded-full 
                                    {{ $eventBooking->room->category === 'Rooms' ? 'bg-blue-900 text-blue-200' : '' }}
                                    {{ $eventBooking->room->category === 'Cottages' ? 'bg-purple-900 text-purple-200' : '' }}
                                    {{ $eventBooking->room->category === 'Event and Dining' ? 'bg-pink-900 text-pink-200' : '' }}">
                                    {{ $eventBooking->room->category ?? 'N/A' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 booking-dates">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-3">
                                    <!-- View Details -->
                                    <a href="{{ route('manager.bookings.show', $eventBooking) }}" 
                                       class="text-blue-400 hover:text-blue-300 transition-colors" title="View Details">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
