...
<main class="relative z-10 py-8 lg:py-16">
    <div class="container mx-auto px-4 lg:px-16">
        ...
        <div class="bg-green-900/50 backdrop-blur-sm rounded-lg p-6 mb-8 border border-green-700/30">
            <form method="GET" action="{{ route('manager.service-requests.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Status Filter -->
                <div>
            ...
            <div class="bg-blue-600/80 backdrop-blur-sm rounded-lg p-6 border border-blue-500/30">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-white">{{ $serviceRequests->where('status', 'pending')->count() }}</p>
                    </div>
            ...
            <div class="bg-yellow-600/80 backdrop-blur-sm rounded-lg p-6 border border-yellow-500/30">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-white">{{ $serviceRequests->where('status', 'in_progress')->count() }}</p>
                    </div>
            ...
            <div class="bg-green-600/80 backdrop-blur-sm rounded-lg p-6 border border-green-500/30">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-white">{{ $serviceRequests->where('status', 'completed')->count() }}</p>
                    </div>
            ...
            <div class="bg-purple-600/80 backdrop-blur-sm rounded-lg p-6 border border-purple-500/30">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-white">{{ $serviceRequests->total() }}</p>
                    </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-green-700/30">
                        ...
                        <tr class="hover:bg-green-800/30 transition-colors duration-150">
                            <td class="px-6 py-4 text-green-50">#{{ $request->id }}</td>
                            <td class="px-6 py-4">
                                <div class="text-green-50 font-medium">{{ $request->guest_name }}</div>
                                <div class="text-green-200 text-sm">{{ $request->guest_email }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-green-50">{{ $request->service ? $request->service->name : 'N/A' }}</div>
                                @if($request->service_type)
                                <div class="text-green-200 text-sm">{{ ucfirst($request->service_type) }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-green-50">{{ $request->room_number }}</td>
                            <td class="px-6 py-4">
                                @php
                                    $statusColors = [
                                ...
                                @endphp
                                <span class="px-3 py-1 rounded-full text-xs font-medium {{ $statusColors[$request->status] ?? 'bg-gray-600 text-gray-100' }}">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <span class="px-3 py-1 rounded-full text-xs font-medium {{ $priorityColors[$request->priority] ?? 'bg-gray-600 text-gray-100' }}">
                                    {{ ucfirst($request->priority) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-green-50 text-sm">
                                {{ $request->requested_at ? $request->requested_at->format('M d, Y H:i') : 'N/A' }}
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex space-x-2">
                                    <a href="{{ route('manager.service-requests.show', $request) }}" 
