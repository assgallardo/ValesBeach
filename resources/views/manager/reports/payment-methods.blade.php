...
<div class="min-h-screen bg-gray-900 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route($routePrefix . '.reports.index', request()->query()) }}" class="text-emerald-400 hover:text-emerald-300 mb-2 inline-flex items-center">
        ...
        <div class="bg-emerald-900/30 border border-emerald-600/30 rounded-lg p-4 mb-8">
            <div class="flex items-center">
                ...
                <span class="text-emerald-100">Data from <strong>{{ $startDate->format('M d, Y') }}</strong> to <strong>{{ $endDate->format('M d, Y') }}</strong></span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-5">
                ...
                <h2 class="text-2xl font-bold text-white mb-1">{{ number_format($stats['total_transactions']) }}</h2>
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-5">
                ...
                <h2 class="text-2xl font-bold text-white mb-1">₱{{ number_format($stats['total_revenue'], 2) }}</h2>
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 p-5">
                ...
                <h2 class="text-2xl font-bold text-white mb-1">₱{{ number_format($stats['avg_transaction'], 2) }}</h2>
            ...
            <div class="bg-gradient-to-br from-emerald-900/40 to-emerald-800/30 rounded-lg border-2 border-emerald-500/50 p-5">
                ...
                <h2 class="text-xl font-bold text-emerald-400 mb-1">{{ ucwords(str_replace('_', ' ', $stats['most_popular_method'])) }}</h2>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-700/50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 bg-emerald-600/20 rounded-lg flex items-center justify-center mr-3">
                                        <i class="fas fa-{{ $method->payment_method === 'cash' ? 'money-bill' : ($method->payment_method === 'card' ? 'credit-card' : 'mobile') }} text-emerald-400"></i>
                                    </div>
                                    <span class="text-white font-medium">{{ ucwords(str_replace('_', ' ', $method->payment_method)) }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-white font-semibold">₱{{ number_format($method->total_amount, 2) }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="text-emerald-400 font-semibold text-sm">{{ number_format($percentage, 1) }}%</span>
                                </div>
                            </td>
                        </tr>
                        @empty
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                    @forelse($paymentsBySource['bookings'] as $payment)
                    <div class="flex justify-between items-center">
                        <span class="text-gray-300 text-sm">{{ ucwords(str_replace('_', ' ', $payment->payment_method)) }}</span>
                        <span class="text-blue-400 font-bold">₱{{ number_format($payment->total, 2) }}</span>
                    </div>
                    @empty
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                    @forelse($paymentsBySource['food_orders'] as $payment)
                    <div class="flex justify-between items-center">
                        <span class="text-gray-300 text-sm">{{ ucwords(str_replace('_', ' ', $payment->payment_method)) }}</span>
                        <span class="text-green-400 font-bold">₱{{ number_format($payment->total, 2) }}</span>
                    </div>
                    @empty
                    <p class="text-gray-500 text-sm text-center py-4">No data</p>
                    @endforelse
            ...
            <div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                    @forelse($paymentsBySource['services'] as $payment)
                    <div class="flex justify-between items-center">
                        <span class="text-gray-300 text-sm">{{ ucwords(str_replace('_', ' ', $payment->payment_method)) }}</span>
                        <span class="text-purple-400 font-bold">₱{{ number_format($payment->total, 2) }}</span>
                    </div>
                    @empty
                    <p class="text-gray-500 text-sm text-center py-4">No data</p>
                    @endforelse
