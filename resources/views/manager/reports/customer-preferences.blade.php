...
<div class="min-h-screen bg-gray-900 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Header -->
        <div class="mb-8">
            <a href="{{ route($routePrefix . '.reports.index', request()->query()) }}" class="text-teal-400 hover:text-teal-300 mb-2 inline-flex items-center">
        ...
        <div class="bg-teal-900/30 border border-teal-600/30 rounded-lg p-4 mb-8">
            <div class="flex items-center">
                ...
                <span class="text-teal-100">Data from <strong>{{ $startDate->format('M d, Y') }}</strong> to <strong>{{ $endDate->format('M d, Y') }}</strong></span>
            </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden mb-8">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @forelse($roomPreferences as $pref)
                    <div class="bg-gray-900 rounded-lg p-4 border border-gray-700">
                        <h4 class="text-white font-semibold mb-2">{{ $pref->category }}</h4>
                        <div class="flex justify-between items-center mb-1">
                            <span class="text-gray-400 text-sm">Bookings:</span>
                            <span class="text-blue-400 font-bold">{{ $pref->booking_count }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-gray-400 text-sm">Unique Customers:</span>
                            <span class="text-green-400 font-bold">{{ $pref->unique_customers }}</span>
                        </div>
                    </div>
                    @empty
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-700">
                        @forelse($servicePreferences as $pref)
                        <tr class="hover:bg-gray-700/50">
                            <td class="px-6 py-4 text-white">{{ $pref->category }}</td>
                            <td class="px-6 py-4 text-gray-300">{{ $pref->name }}</td>
                            <td class="px-6 py-4"><span class="px-3 py-1 bg-purple-900/30 text-purple-400 rounded-full text-sm font-semibold">{{ $pref->request_count }}</span></td>
                            <td class="px-6 py-4 text-gray-400">{{ $pref->unique_customers }}</td>
                        </tr>
                        @empty
        ...
        <div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden mb-8">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-700">
                        @forelse($foodPreferences as $pref)
                        <tr class="hover:bg-gray-700/50">
                            <td class="px-6 py-4 text-white">{{ $pref->category }}</td>
                            <td class="px-6 py-4 text-gray-300">{{ $pref->item_name }}</td>
                            <td class="px-6 py-4"><span class="px-3 py-1 bg-green-900/30 text-green-400 rounded-full text-sm font-semibold">{{ $pref->total_orders }}</span></td>
                            <td class="px-6 py-4 text-gray-400">{{ $pref->unique_customers }}</td>
                        </tr>
                        @empty
        ...
        <div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
                    @forelse($bookingTimes as $time)
                    <div class="bg-gray-900 rounded-lg p-4 text-center border border-gray-700">
                        <div class="text-gray-400 text-sm mb-2">{{ $time->day_name }}</div>
                        <div class="text-yellow-400 text-2xl font-bold">{{ $time->booking_count }}</div>
