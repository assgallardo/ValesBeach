...
<div class="max-w-5xl mx-auto">
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-400 mt-1">Add a new service offering to the resort</p>
        </div>
    ...
    <div class="bg-gray-800 rounded-lg shadow-xl overflow-hidden">
        <form action="{{ route('manager.services.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="category" 
                                    name="category"
                                    class="w-full bg-gray-700 text-white rounded-lg px-4 py-2.5 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent @error('category') border-red-500 @enderror"
                                    required>
                                <option value="">Select Category</option>
                                <option value="spa" {{ old('category') === 'spa' ? 'selected' : '' }}>Spa & Wellness</option>
                                <option value="dining" {{ old('category') === 'dining' ? 'selected' : '' }}>Dining</option>
                                ...
                                <option value="room_service" {{ old('category') === 'room_service' ? 'selected' : '' }}>Room Service</option>
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                            @enderror
