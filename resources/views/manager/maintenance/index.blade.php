                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-white">
                                {{ \App\Models\ServiceRequest::where('status', 'pending')->where('service_id', function($query) {
                ...
                <div class="bg-blue-600 rounded-lg p-6 shadow-lg">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-white">
                                {{ \App\Models\ServiceRequest::whereIn('status', ['confirmed', 'in_progress'])->where('service_id', function($query) {
                                    $query->select('id')->from('services')->where('category', 'maintenance');
                                })->count() }}
                            </p>
                        </div>
                ...
                <div class="bg-green-600 rounded-lg p-6 shadow-lg">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-white">
                                {{ \App\Models\ServiceRequest::where('status', 'completed')->where('service_id', function($query) {
                                    $query->select('id')->from('services')->where('category', 'maintenance');
                                })->count() }}
                            </p>
                        </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            @forelse($maintenanceRequests as $request)
                                <tr>
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $request->service->name ?? 'N/A' }}</div>
                                        @if($request->description)
                                            <div class="text-sm text-gray-500 truncate max-w-xs">{{ $request->description }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">Room {{ $request->room_number ?? 'N/A' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('manager.service-requests.show', $request->id) }}" 
