...
<div class="max-w-7xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-gradient-to-br from-blue-600 to-blue-800 rounded-lg shadow-xl p-6 text-white transform hover:scale-105 transition-transform duration-200">
            ...
            <p class="text-3xl font-bold mb-3">{{ $stats['today']['orders'] }}</p>
            <div class="flex items-center justify-between text-sm">
                <span class="text-blue-100">Revenue</span>
                <span class="font-semibold">₱{{ number_format($stats['today']['revenue'], 2) }}</span>
            </div>
            <div class="flex items-center justify-between text-sm mt-1">
                <span class="text-blue-100">Completed</span>
                <span class="font-semibold">{{ $stats['today']['completed'] }}</span>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-purple-600 to-purple-800 rounded-lg shadow-xl p-6 text-white transform hover:scale-105 transition-transform duration-200">
            ...
            <p class="text-3xl font-bold mb-3">{{ $stats['week']['orders'] }}</p>
            <div class="flex items-center justify-between text-sm">
                <span class="text-purple-100">Revenue</span>
                <span class="font-semibold">₱{{ number_format($stats['week']['revenue'], 2) }}</span>
            </div>
            <div class="flex items-center justify-between text-sm mt-1">
                <span class="text-purple-100">Completed</span>
                <span class="font-semibold">{{ $stats['week']['completed'] }}</span>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-orange-600 to-orange-800 rounded-lg shadow-xl p-6 text-white transform hover:scale-105 transition-transform duration-200">
            ...
            <p class="text-3xl font-bold mb-3">{{ $stats['month']['orders'] }}</p>
            <div class="flex items-center justify-between text-sm">
                <span class="text-orange-100">Revenue</span>
                <span class="font-semibold">₱{{ number_format($stats['month']['revenue'], 2) }}</span>
            </div>
            <div class="flex items-center justify-between text-sm mt-1">
                <span class="text-orange-100">Completed</span>
                <span class="font-semibold">{{ $stats['month']['completed'] }}</span>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-green-600 to-green-800 rounded-lg shadow-xl p-6 text-white transform hover:scale-105 transition-transform duration-200">
            ...
            <p class="text-3xl font-bold mb-3">{{ $stats['all_time']['orders'] }}</p>
            <div class="flex items-center justify-between text-sm">
                <span class="text-green-100">Revenue</span>
                <span class="font-semibold">₱{{ number_format($stats['all_time']['revenue'], 2) }}</span>
            </div>
            <div class="flex items-center justify-between text-sm mt-1">
                <span class="text-green-100">Completed</span>
                <span class="font-semibold">{{ $stats['all_time']['completed'] }}</span>
            </div>
        </div>
    </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-700">
                            @forelse($recentOrders as $order)
                                <tr class="hover:bg-gray-700 transition-colors duration-150">
                                    <td class="py-3 px-2">
                                        <a href="{{ route('staff.orders.show', $order) }}" class="text-blue-400 hover:text-blue-300 font-medium">
                                            {{ $order->order_number }}
                                        </a>
                                    </td>
                                    <td class="py-3 px-2 text-gray-300">{{ $order->customer_name }}</td>
                                    <td class="py-3 px-2 text-center text-gray-300">{{ $order->orderItems->count() }}</td>
                                    ...
                                    <td class="py-3 px-2 text-right text-white font-semibold">₱{{ number_format($order->total_amount, 2) }}</td>
                                </tr>
                            @empty
                                <tr>
