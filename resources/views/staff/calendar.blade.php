                ...
                <div class="min-h-[120px] border-r border-b border-gray-700 last:border-r-0 p-2 {{ $isCurrentMonth ? 'bg-gray-800' : 'bg-gray-900' }} {{ $isToday ? 'ring-2 ring-blue-500' : '' }}">
                    ...
                    <div class="space-y-1">
                        @foreach($dayBookings->take(3) as $booking)
                            ...
                            @endphp
                            <div class="text-xs {{ $categoryColor }} text-white p-1 rounded cursor-pointer hover:opacity-90 transition-opacity {{ $opacity }}"
                                 onclick="showBookingDetails({{ $booking->id }})"
                                 title="{{ $booking->room->name }} - {{ $booking->user->name }} - {{ ucfirst(str_replace('_', ' ', $booking->status)) }}">
                                <div class="truncate font-medium">{{ $booking->room->name }}</div>
                                <div class="truncate text-xs opacity-90">{{ $booking->user->name }}</div>
                            </div>
                        @endforeach
    ...
    const content = `
        <div class="space-y-4">
            ...
            <div>
                ...
                ${(booking.room.check_in_time && booking.room.check_in_time !== '00:00:00') ? `<p class="text-green-400 text-sm mt-1"><i class="fas fa-clock mr-1"></i>Check-in time: ${new Date('2000-01-01 ' + booking.room.check_in_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}</p>` : ''}
            </div>
            <div>
                ...
                ${(booking.room.check_out_time && booking.room.check_out_time !== '00:00:00') ? `<p class="text-yellow-400 text-sm mt-1"><i class="fas fa-clock mr-1"></i>Check-out time: ${new Date('2000-01-01 ' + booking.room.check_out_time).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}</p>` : ''}
            </div>
            <div>
