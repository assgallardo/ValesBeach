            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($groupedItems as $categoryId => $items)
                    ...
                    <div class="category-card bg-gray-800 rounded-xl shadow-xl overflow-hidden border border-gray-700 hover:border-green-500 transition-all duration-300 transform hover:scale-105 cursor-pointer"
                         onclick="showCategory('{{ $categoryId }}', '{{ $categoryName }}', '{{ $colors['gradient'] }}', '{{ $colors['icon'] }}')">
                        <div class="bg-gradient-to-br {{ $colors['gradient'] }} p-8 text-center relative overflow-hidden">
                            ...
                            <div class="relative z-10">
                                <div class="w-20 h-20 mx-auto mb-4 bg-white bg-opacity-20 rounded-full flex items-center justify-center backdrop-blur-sm">
                                    <i class="fas {{ $colors['icon'] }} text-white text-3xl"></i>
                                </div>
                                <h3 class="text-2xl font-bold text-white mb-2">{{ $categoryName }}</h3>
                                <p class="text-white text-sm opacity-90">{{ $items->count() }} {{ Str::plural('item', $items->count()) }}</p>
                            </div>
                        </div>
...
<script>
const allMenuItems = @json($allItems);
