...
<div class="min-h-screen bg-gray-900 py-8">
    <div class="container mx-auto px-4 lg:px-8 max-w-7xl">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start mb-8 gap-4">
            <div>
                <h1 class="text-4xl font-bold text-white mb-2">Order #{{ $foodOrder->order_number }}</h1>
                <p class="text-gray-400">Placed on {{ $foodOrder->created_at->format('M j, Y \a\t g:i A') }}</p>
            </div>
                ...
                <div class="bg-gray-800 rounded-xl shadow-2xl p-6">
                    ...
                    <div class="flex items-center space-x-4 mb-6">
                        <div class="flex-shrink-0">
                            ...
                            <div class="w-16 h-16 bg-{{ $config['color'] }}-900 rounded-full flex items-center justify-center">
                                @if($config['icon'] === 'clock')
                                <svg class="w-8 h-8 text-{{ $config['color'] }}-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-white capitalize mb-2">{{ str_replace('_', ' ', $foodOrder->status) }}</h3>
                    ...
                    <div class="mt-8 p-4 bg-gray-700/30 rounded-lg">
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center text-sm">
                                ...
                                <span class="text-gray-400">{{ $foodOrder->created_at->format('g:i A') }}</span>
                            </div>
                            ...
                            <div class="flex items-center text-sm">
                                ...
                                <span class="text-gray-400">{{ $foodOrder->confirmed_at->format('g:i A') }}</span>
                            </div>
                            @endif
                            ...
                            <div class="flex items-center text-sm">
                                ...
                                <span class="text-gray-400">{{ $foodOrder->prepared_at->format('g:i A') }}</span>
                            </div>
                            @endif
                            ...
                            <div class="flex items-center text-sm">
                                ...
                                <span class="text-gray-400">{{ $foodOrder->delivered_at->format('g:i A') }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-start space-x-4 p-4 bg-gray-700/30 rounded-lg hover:bg-gray-700/50 transition-colors">
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="font-bold text-lg text-white mb-1">{{ $item->menuItem->name }}</h3>
                                @if($item->menuItem->description)
                                <p class="text-gray-300 text-sm mb-2">{{ $item->menuItem->description }}</p>
                                @endif
                ...
                <div class="bg-gray-800 rounded-xl shadow-2xl p-6 sticky top-4 border border-gray-700">
                    ...
                    <div class="mb-6 p-4 bg-gray-700/30 rounded-lg">
                        ...
                        <div class="text-sm text-gray-300 space-y-2">
                            <p><strong class="text-white">Type:</strong> {{ str_replace('_', ' ', ucfirst($foodOrder->delivery_type)) }}</p>
                            @if($foodOrder->delivery_location)
                            <p><strong class="text-white">Location:</strong> {{ $foodOrder->delivery_location }}</p>
                            @endif
                            @if($foodOrder->requested_delivery_time)
                            <p><strong class="text-white">Requested Time:</strong> {{ $foodOrder->requested_delivery_time->format('M j, g:i A') }}</p>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mb-6 p-4 bg-yellow-900/30 border border-yellow-600/50 rounded-lg">
                        ...
                        <p class="text-sm text-yellow-100">{{ $foodOrder->special_instructions }}</p>
                    </div>
                    @endif
                    ...
                    <div class="border-t border-gray-700 pt-4 space-y-3">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-semibold text-white">{{ $foodOrder->formatted_subtotal }}</span>
                        </div>
                        @if($foodOrder->delivery_fee > 0)
                        <div class="flex justify-between items-center">
                            <span class="text-gray-300">Delivery Fee</span>
                            <span class="font-semibold text-white">{{ $foodOrder->formatted_delivery_fee }}</span>
                        </div>
                        @endif
                        <div class="flex justify-between items-center">
                            <span class="text-gray-300">Tax</span>
                            <span class="font-semibold text-white">{{ $foodOrder->formatted_tax_amount }}</span>
                        </div>
                        <div class="border-t border-gray-700 pt-3">
                            <div class="flex justify-between items-center p-4 bg-gradient-to-r from-green-900/50 to-green-800/50 rounded-lg">
                                ...
                                <span class="text-2xl font-bold text-green-400">{{ $foodOrder->formatted_total_amount }}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-700/30 rounded-lg">
                            ...
                            <span class="px-3 py-1.5 text-sm rounded-full font-bold
                                @switch($foodOrder->payment_status)
                                    ...
                                    @case('paid') bg-green-600 text-green-100 @break
                                    @case('failed') bg-red-600 text-red-100 @break
                                    @case('refunded') bg-gray-600 text-gray-100 @break
                                @endswitch">
                    ...
                    <div class="mt-6 p-4 bg-blue-900/30 border border-blue-600/50 rounded-lg">
                        ...
                        <p class="text-blue-100 text-sm space-y-1">
                            <span class="block"><strong>Booking:</strong> {{ $foodOrder->booking->booking_number }}</span>
                            @if($foodOrder->booking->room)
                            <span class="block"><strong>Room:</strong> {{ $foodOrder->booking->room->room_number }}</span>
                            @endif
                        </p>
                    </div>
                    @endif
