                ...
                @endif
                            ...
                            <div class="p-4 bg-gray-800/90 backdrop-blur-sm">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <span class="text-xl font-bold text-yellow-400 ml-2">₱{{ number_format($item->price, 2) }}</span>
                    </div>

                    @if($item->description)
                                <p class="text-gray-300 text-xs mb-2 line-clamp-2">{{ $item->description }}</p>
                    @endif
                    
                    <!-- Dietary Badges -->
                                <div class="flex flex-wrap gap-1.5 mb-3">
                        @foreach($item->dietary_badges as $badge)
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $badge['class'] }}">
                            {{ $badge['label'] }}
                        </span>
                        @endforeach
                                    @if($item->calories)
    ...
    @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div onclick="showCategory({{ $category->id }}, '{{ $category->name }}')" 
                         class="group cursor-pointer bg-gradient-to-br {{ $data['gradient'] }} rounded-2xl p-8 shadow-2xl hover:shadow-3xl transform hover:scale-105 transition-all duration-300 relative overflow-hidden">
                        <!-- Background Pattern -->
                        ...
                        <div class="relative z-10">
                            ...
                            <p class="text-white/90 text-sm mb-4">{{ $category->description }}</p>
                            @endif
        ...
        <div id="menu-data" class="hidden">
            ...
            <div data-category-id="{{ $category->id }}" data-category-name="{{ $category->name }}">
            @foreach($category->menuItems as $item)
                <div class="menu-item-data" data-item-id="{{ $item->id }}">
                    ...
                    <input type="hidden" name="item_allergens" value="{{ is_array($item->allergens) ? implode(', ', $item->allergens) : '' }}">
                    <input type="hidden" name="item_badges" value="{{ json_encode($item->dietary_badges ?? []) }}">
                </div>
                @endforeach
            </div>
            @endforeach
