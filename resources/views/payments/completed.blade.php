...
<div class="min-h-screen bg-gray-900 py-6">
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @else
            <!-- Stats Summary -->
            <div class="mb-6">
                <p class="text-gray-400 text-right">Showing 1 to {{ $paymentTransactions->count() }} of {{ $paymentTransactions->count() }} payment transaction{{ $paymentTransactions->count() > 1 ? 's' : '' }}</p>
            </div>
                        ...
                        <tr class="hover:bg-gray-750 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex flex-col gap-1">
                                    @if($transaction->bookings_count > 0)
                                        <div class="flex items-center gap-1.5">
                                            <i class="fas fa-bed text-blue-400 text-xs"></i>
                                            <span class="text-xs text-gray-300">{{ $transaction->bookings_count }} Booking{{ $transaction->bookings_count > 1 ? 's' : '' }}</span>
                                        </div>
                                    @endif
                                    @if($transaction->services_count > 0)
                                        <div class="flex items-center gap-1.5">
                                            <i class="fas fa-concierge-bell text-purple-400 text-xs"></i>
                                            <span class="text-xs text-gray-300">{{ $transaction->services_count }} Service{{ $transaction->services_count > 1 ? 's' : '' }}</span>
                                        </div>
                                    @endif
                                    @if($transaction->food_orders_count > 0)
                                        <div class="flex items-center gap-1.5">
                                            <i class="fas fa-utensils text-orange-400 text-xs"></i>
                                            <span class="text-xs text-gray-300">{{ $transaction->food_orders_count }} Food Order{{ $transaction->food_orders_count > 1 ? 's' : '' }}</span>
                                        </div>
                                    @endif
                                    @if($transaction->extra_charges_count > 0)
                                        <div class="flex items-center gap-1.5">
                                            <i class="fas fa-receipt text-yellow-400 text-xs"></i>
                                            <span class="text-xs text-gray-300">{{ $transaction->extra_charges_count }} Extra Charge{{ $transaction->extra_charges_count > 1 ? 's' : '' }}</span>
                                        </div>
                                    @endif
                                </div>
                            </td>
                            ...
                            <!-- Total Amount -->
                            <td class="px-4 py-4">
                                <div class="text-lg font-bold text-green-400">₱{{ number_format($transaction->totalAmount, 2) }}</div>
                            </td>

                            <!-- Payments Count -->
                            <td class="px-4 py-4">
                                <div class="text-sm text-gray-300">{{ $transaction->totalTransactions }} payment{{ $transaction->totalTransactions > 1 ? 's' : '' }}</div>
                            </td>

                            <!-- Status -->
                            <td class="px-4 py-4">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-600/20 text-white">
                                    {{ $transaction->totalTransactions }} Completed
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-4">
                                <div class="text-sm text-gray-300">
                                    {{ \Carbon\Carbon::parse($transaction->latestDate)->format('M d, Y') }}
                                </div>
                                <div class="text-xs text-gray-400">
                                    {{ \Carbon\Carbon::parse($transaction->latestDate)->format('h:i A') }}
                                </div>
                            </td>

                            <!-- Actions -->
                            <td class="px-4 py-4">
                                <a href="{{ route('payments.completed.details', ['transaction_id' => $transaction->payment_transaction_id]) }}"
