...
<div class="container mx-auto px-4 lg:px-8 py-8">
    ...
    <div class="mb-8 flex items-center">
        ...
        <div>
            <h1 class="text-3xl font-bold text-white">Create Manual Booking</h1>
            <p class="text-gray-400 mt-2">Add a new booking for a guest</p>
        </div>
    </div>
                    ...
                    <select name="room_id" required id="room_select"
                        ...
                        @foreach($rooms as $room)
                            <option value="{{ $room->id }}" 
                                ...
                                {{ $room->name }} - ₱{{ number_format($room->price_per_night, 2) }}/night ({{ $room->capacity }} guests max)
                            </option>
                        @endforeach
                    </select>
                    @error('room_id')
