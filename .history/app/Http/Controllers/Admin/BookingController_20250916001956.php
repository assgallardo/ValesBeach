...
class BookingController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->has('booking_id') && $request->has('new_status')) {
                ...
                if ($booking) {
                    ...
                    $booking->update(['status' => $request->new_s        <div class="page-header">
        ...
        if ($successMessage) {
            $html .= '
        <div style="background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.3); border-radius: 8px; padding: 16px; margin-bottom: 24px; color: rgb(187, 247, 208);">
            ' . $successMessage . '
        </div>';
        }
